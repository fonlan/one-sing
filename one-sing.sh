...
delete_inbound() {
    ...
    # 从配置文件中获取所有协议信息
    local inbounds
    inbounds=$(jq -r '.inbounds[] | "\(.type)|\(.listen_port)"' "$SING_CONFIG")
    if [ -z "$inbounds" ]; then
...
list_inbounds() {
    ...
    # 从配置文件中获取所有协议信息
    local inbounds
    inbounds=$(jq -r '.inbounds[]' "$SING_CONFIG")
    if [ -z "$inbounds" ]; then
        echo -e "${YELLOW}还没有添加任何协议${NC}"
        return
    fi
    ...
    jq -c '.inbounds[]' "$SING_CONFIG" | while read -r inbound; do
        ...
        "vless")
            ...
            local flow
            flow=$(echo "$inbound" | jq -r '.users[0].flow')
            local server_name
        ...
        "anytls")
            ...
            local cert_path
            cert_path=$(echo "$inbound" | jq -r '.tls.certificate_path')
            local cert_fingerprint=""
